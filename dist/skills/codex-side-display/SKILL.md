...
4. 默认按局域网 Sidecar 模式部署：设备和主机应在同一 Wi-Fi，避免依赖 `adb reverse` 掩盖网络问题。
5. 安装后截图验证：界面应显示真实 Spotify 或 Apple Music 信息、专辑封面、Codex `5 小时` 与 `1 周` 剩余用量。
...
- 想脱离 USB 线使用：确认 Android 和主机在同一局域网，并用 `adb -s <serial> reverse --list` 检查没有残留端口转发。
